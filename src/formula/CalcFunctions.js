...
export function createFunctions(simulate) {
  ...
  defineFunction(
    ...
    "RandDist",
    {
      params: [
        ...
        {
          name: "Y (in which case Distribution is X)",
          noUnits: true,
          needVector: true,
          defaultVal: false,
        },
    ...
    (x) => {
      ...
      if (x.length === 1 || x[1] === false) {
        ...
        for (
          ...
          if (
            !(vec.items[i] instanceof Vector) ||
            vec.items[i].items.length !== 2
          ) {
            throw new ModelError(
              "Invalid vector provided to RandDist.",
              {
                code: 6000,
              },
            );
          ...
          xVals.push(
            toNum(vec.items[i].items[0]).value,
          );
          yVals.push(
            toNum(vec.items[i].items[1]).value,
          );
        }
      } else {
        xVals = toNum(x[0]).items.map(
          (x) => x.value,
        );
        yVals = toNum(x[1]).items.map(
          (x) => x.value,
        );
      }
      return new Material(
        RandDist(simulate, xVals, yVals),
      );
    },
  );
  ...
  defineFunction(
    ...
    (x) => {
      if (x.length === 0) {
        ...
        throw new ModelError(
          "Rand() must either have no parameters or two: Min and Max bounds.",
          {
            code: 6001,
          },
        );
  ...
  defineFunction(
    ...
    (x) => {
      if (x.length === 0) {
        ...
        throw new ModelError(
          "RandNormal() must either have no parameters or two: Mean and Standard Deviation.",
          {
            code: 6002,
          },
        );
  ...
  defineFunction(
    ...
    (x) => {
      if (x.length !== 0) {
        return new Material(
          RandExp(simulate, toNum(x[0]).value),
        );
      } else {
  ...
  defineFunction(
    ...
    (x) => {
      return new Material(
        RandPoisson(simulate, toNum(x[0]).value),
      );
    },
  );
  defineFunction(
  ...
  defineFunction(
    ...
    {
      params: [
        { name: "Vector", needVector: true },
      ],
    },
    (x) => {
      return simulate.coreBank.get("sqrt")([
        simulate.coreBank.get("sum")([
          mult(x[0], x[0]),
        ]),
      ]);
    },
  ...
  defineFunction(
    ...
    (x) => {
      ...
      return new Material(
        fn.asin(val),
        simulate.unitManager.getUnitStore(
          ["radians"],
          [1],
        ),
        false,
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (r.value < 0) {
        throw new ModelError(
          "Sqrt() requires a number greater than or equal to 0.",
          {
            code: 6007,
          },
        );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (val < 0) {
        throw new ModelError(
          "Ln() requires a number greater than or equal to 0. Got, <i>" +
            val +
            "</i>.",
          {
            code: 6008,
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (val < 0) {
        throw new ModelError(
          "Log() requires a number greater than or equal to 0. Got, <i>" +
            val +
            "</i>.",
          {
            code: 6009,
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (r.value < 0 || r.value > 1) {
        throw new ModelError(
          "Logit() requires an input between 0 and 1 (inclusive).",
          {
            code: 6079,
          },
        );
  ...
  defineFunction(
    ...
    (x) => {
      return new Material(
        fn.exp(toNum(x[0]).value),
      );
    },
  );

  simulate.varBank.set(
    "ifthenelse",
    function (x) {
  ...
  simulate.varBank.get(
    "ifthenelse",
  ).delayEvalParams = true;
  simulate.coreBank.set(
    "ifthenelse",
    simulate.varBank.get("ifthenelse"),
  );
  ...
  simulate.varBank.get("assert").delayEvalParams =
    true;
  simulate.coreBank.set(
    "assert",
    simulate.varBank.get("assert"),
  );
  ...
  function vecIfThenElse(test, tVal, fVal) {
    ...
    let testFn = function (test, val) {
      ...
      if (!Array.isArray(val)) {
        throw new ModelError(
          "Keys do not match for vectorized IfThenElse().",
          {
            code: 6093,
          },
        );
  ...
  simulate.varBank.set("map", function (x) {
    ...
    if (!(v instanceof Vector)) {
      throw new ModelError(
        "Map() requires a vector as its first argument.",
        {
          code: 6010,
        },
      );
  ...
  simulate.varBank.get("map").delayEvalParams =
    true;
  VectorObject["map"] =
    simulate.varBank.get("map");
  simulate.coreBank.set(
    "map",
    simulate.varBank.get("map"),
  );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (length === 0) {
        throw new ModelError(
          "Sample() requires a non-empty vector.",
          {
            code: 6011,
          },
        );
      ...
      if (repeat) {
        for (let i = 0; i < count; i++) {
          res.push(
            v.items[
              Math.floor(Rand(simulate) * length)
            ],
          );
        }
      } else {
        if (length < count) {
          throw new ModelError(
            "Vector for Sample() is too small for the given sample size.",
            {
              code: 6012,
            },
          );
  ...
  defineFunction(
    ...
    "Collapse",
    {
      params: [
        ...
        { name: "Target", noVector: false },
      ],
    },
    (x) => {
      return toNum(x[0]).collapseDimensions(
        toNum(x[1]),
      );
    },
  );
  ...
  simulate.varBank.set("filter", function (x) {
    ...
    if (!(v instanceof Vector)) {
      throw new ModelError(
        "Filter() requires a vector as its first argument.",
        {
          code: 6013,
        },
      );
    ...
    return simulate.coreBank.get("select")([
      v,
      t,
    ]);
  });
  simulate.varBank.get("filter").delayEvalParams =
    true;
  VectorObject["filter"] =
    simulate.varBank.get("filter");
  simulate.coreBank.set(
    "filter",
    simulate.varBank.get("filter"),
  );
  ...
  simulate.varBank.set("join", function (x) {
    ...
    return new Vector(
      res,
      simulate,
      hasNames ? names : undefined,
    );
  });
  simulate.coreBank.set(
    "join",
    simulate.varBank.get("join"),
  );
  ...
  simulate.varBank.set("repeat", function (x) {
    ...
    if (items instanceof Vector) {
      if (items.names && items.names.length) {
        throw new ModelError(
          `If a repeat count is a vector, it can't have names. Got, <i>${items}</i>.`,
          {
            code: 6070,
          },
        );
      ...
      if (
        ...
        throw new ModelError(
          `If a repeat count is a vector, it must consist of all strings. Got, <i>${items}</i>.`,
          {
            code: 6071,
          },
        );
      ...
      if (
        new Set(innerItems).size !==
        innerItems.length
      ) {
        throw new ModelError(
          `If a repeat count is a vector, it must contain unique strings. Got, <i>${items}</i>.`,
          {
            code: 6072,
          },
        );
      ...
      if (items.value < 0) {
        throw new ModelError(
          `Repeat count must be a non-negative. Got, <i>${items.value}</i>.`,
          {
            code: 6075,
          },
        );
      ...
      throw new ModelError(
        `Repeat count must be a Number or Vector. Got, <i>${items}</i>.`,
        {
          code: 6074,
        },
      );
    ...
    for (let i = 0; i < count; i++) {
      ...
      res.push(
        evaluateNode(x[0].node, scope, simulate),
      );
    }
    ...
    return new Vector(
      res,
      simulate,
      items instanceof Vector
        ? items.items.slice()
        : undefined,
    );
  });
  simulate.varBank.get("repeat").delayEvalParams =
    true;
  simulate.coreBank.set(
    "repeat",
    simulate.varBank.get("repeat"),
  );
      ...
      if (x[1] instanceof Vector) {
        ...
        if (isBoolean === true) {
          ...
          if (v.length() !== x[0].length()) {
            throw new ModelError(
              "Length of vector must be equal for boolean selection.",
              {
                code: 6014,
              },
            );
  ...
  defineFunction(
    ...
    (x) => {
      return new Vector(
        x.items.slice().reverse(),
        simulate,
        x.names
          ? x.names.slice().reverse()
          : undefined,
      );
    },
  );
  ...
  defineFunction(
    ...
    "Union",
    {
      object: [simulate.varBank, VectorObject],
      params: [
        { name: "Vector 1", needVector: true },
        { name: "Vector 2", needVector: true },
      ],
    },
    (x) => {
      return simulate.coreBank.get("unique")(
        simulate.coreBank.get("join")(x).items,
      );
    },
  );
    ...
    (x) => {
      ...
      for (let i = 0; i < v1.items.length; i++) {
        for (
          ...
          if (
            strictEquals(v1.items[i], v2.items[j])
          ) {
            res.push(v1.items[i]);
            break;
          }
    ...
    (x) => {
      ...
      for (let i = 0; i < v2.items.length; i++) {
        ...
        for (
          ...
          if (
            strictEquals(v2.items[i], v1.items[j])
          ) {
            found = true;
            break;
          }
    ...
    (x) => {
      let res = x.stackApply((v) => {
        ...
        if (x.length > 0) {
          ...
          throw new ModelError(
            "You must have at least one element to calculate a max.",
            {
              code: 6017,
            },
          );
  ...
  function joinVector(x, notToNum, skip) {
    ...
    if (
      x.length === 1 &&
      x[0] instanceof Vector
    ) {
      if (skip) {
        return x[0];
      } else {
        return simulate.coreBank.get("flatten")([
          x[0],
        ]);
      }
    } else {
  ...
  function replicateVectorStructure(vec, val) {
    ...
    for (let i = 0; i < v.items.length; i++) {
      if (v.items[i] instanceof Vector) {
        v.items[i] = replicateVectorStructure(
          v.items[i],
          val,
        );
      } else {
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (
        xVec.items.length !== yVec.items.length
      ) {
        throw new ModelError(
          "The <i>value</i> and <i>results</i> vectors must be the same length",
          {
            code: 6018,
          },
        );
      ...
      if (xVec.items.length < 1) {
        throw new ModelError(
          "You must have at least one element in your vectors",
          {
            code: 6019,
          },
        );
    ...
    (x) => {
      let res = x.stackApply((v) => {
        ...
        if (x.length > 0) {
          ...
          throw new ModelError(
            "You must have at least one element to calculate a min.",
            {
              code: 6020,
            },
          );
  ...
  defineFunction(
    ...
    (x) => {
      let res = x.stackApply((v) => {
        /** @type {Material[]} */
        let x = simulate.coreBank.get("sort")([
          v,
        ]).items;
        if (x.length > 0) {
          if (
            Math.floor((x.length - 1) / 2) ===
            (x.length - 1) / 2
          ) {
          ...
          throw new ModelError(
            "You must have at least one element to calculate a median.",
            {
              code: 6021,
            },
          );
    ...
    (x) => {
      let res = x.stackApply((v) => {
        ...
        if (x.length > 1) {
          ...
          let r = power(
            div(sum, new Material(x.length - 1)),
            new Material(0.5),
          );
          ...
          throw new ModelError(
            "You must have at least two elements to calculate the standard deviation.",
            {
              code: 6022,
            },
          );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (v1.length() <= 1) {
        throw new ModelError(
          "You must have at least two elements in your vectors to calculate their correlation.",
          {
            code: 6023,
          },
        );
      ...
      if (v1.length() !== v2.length()) {
        throw new ModelError(
          "The vectors for Correlation() must be of the same size.",
          {
            code: 6024,
          },
        );
      ...
      let v1Mean = simulate.coreBank.get("mean")([
        v1,
      ]);
      let v2Mean = simulate.coreBank.get("mean")([
        v2,
      ]);

      let v1StdDev = simulate.coreBank.get(
        "stddev",
      )([v1]);
      let v2StdDev = simulate.coreBank.get(
        "stddev",
      )([v2]);
      ...
      return div(
        simulate.coreBank.get("sum")([
          mult(
            minus(v1.clone(), v1Mean),
            minus(v2.clone(), v2Mean),
          ),
        ]),
        mult(
          minus(
            simulate.coreBank.get("count")([v1]),
            new Material(1),
          ),
          mult(v1StdDev, v2StdDev),
        ),
      );
    },
  );
  ...
  simulate.varBank.set("flatten", function (x) {
    ...
    return new Vector(
      res.items,
      simulate,
      res.hasName ? res.names : undefined,
    );
  });
  simulate.coreBank.set(
    "flatten",
    simulate.varBank.get("flatten"),
  );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (val < 0 || val > 1) {
        throw new ModelError(
          "<i>p</i> is a probability and must be between 0 and 1 inclusive.",
          {
            code: 6025,
          },
        );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (dof <= 0) {
        throw new ModelError(
          "<i>Degrees of Freedom</i> must be greater than 0.",
          {
            code: 6027,
          },
        );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      return new Material(
        jStat.centralF.cdf(val, dof1, dof2),
      );
    },
  );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (dof1 <= 0) {
        throw new ModelError(
          "<i>Degrees of Freedom 1</i> must be greater than 0.",
          {
            code: 6033,
          },
        );
      ...
      if (dof2 <= 0) {
        throw new ModelError(
          "<i>Degrees of Freedom 2</i> must be greater than 0.",
          {
            code: 6034,
          },
        );
      ...
      return new Material(
        jStat.centralF.pdf(val, dof1, dof2),
      );
    },
  );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      return new Material(
        jStat.centralF.inv(val, dof1, dof2),
      );
    },
  );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (rate <= 0) {
        throw new ModelError(
          "<i>Rate</i> must be greater than 0.",
          {
            code: 6042,
          },
        );
      ...
      return new Material(
        jStat.exponential.cdf(val, rate),
      );
    },
  );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      return new Material(
        jStat.exponential.pdf(val, rate),
      );
    },
  );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      return new Material(
        jStat.exponential.inv(val, rate),
      );
    },
  );
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (Lambda <= 0) {
        throw new ModelError(
          "<i>Lambda</i> must be greater than 0.",
          {
            code: 6046,
          },
        );
  ...
  defineFunction(
    ...
    (x) => {
      simulate.random = SeedRandom.seedrandom(
        toNum(x[0]).value,
      );
      return stringify(
        "Random Seed Set",
  ...
  defineFunction(
    ...
    (x) => {
      if (typeof alert === "undefined") {
        throw new ModelError(
          "Alert() is not implemented on this platform.",
          {
            code: 6048,
          },
        );
  ...
  defineFunction(
    ...
    (x) => {
      if (typeof prompt === "undefined") {
        throw new ModelError(
          "Prompt() is not implemented on this platform.",
          {
            code: 6049,
          },
        );
      ...
      if (result === null) {
        ...
        parseFloat(result).toString() === result
      ) {
  ...
  defineFunction(
    ...
    (x) => {
      ...
      if (typeof confirm === "undefined") {
        throw new ModelError(
          "Confirm() is not implemented on this platform.",
          {
            code: 6050,
          },
        );
  ...
  defineFunction(
    ...
    "Split",
    {
      object: StringObject,
      params: [
        { name: "String", needString: true },
        { name: "Splitter", needString: true },
      ],
    },
    (x) => {
      return stringify(
        new Vector(x[0].split(x[1]), simulate),
        simulate,
      );
    },
  );
  ...
  defineFunction(
    ...
    "Join",
    {
      object: VectorObject,
      params: [
        { name: "String", needVector: true },
        { name: "Joiner", needString: true },
      ],
    },
    (x) => {
      return stringify(
    ...
    (x) => {
      if (x[1] instanceof Vector) {
        ...
        for (
          ...
          res += x[0].charAt(
            toNum(x[1].items[i]).value - 1,
          );
        }
        return stringify(res, simulate);
      } else {
        return stringify(
          x[0].charAt(toNum(x[1]).value - 1),
          simulate,
        );
      }
    },
  ...
  defineFunction(
    ...
    "IndexOf",
    {
      object: StringObject,
      params: [
        { name: "Haystack", needString: true },
        { name: "Needle", needString: true },
      ],
    },
    (x) => {
  ...
  simulate.coreBank.set(
    "stringbase",
    makeObjectBase(StringObject, simulate),
  );
  simulate.varBank.set(
    "stringbase",
    simulate.coreBank.get("stringbase"),
  );
  simulate.coreBank.set(
    "vectorbase",
    makeObjectBase(VectorObject, simulate),
  );
  simulate.varBank.set(
    "vectorbase",
    simulate.coreBank.get("vectorbase"),
  );
}
...
export function defineFunction(
    ...
    for (
      ...
      if (definition.params[i].vectorize) {
        ...
        if (definition.params[i].noVector) {
          throw new ModelError(
            `Cannot have a non-vector vectorized parameter. Function '${name}', parameter '${definition.params[i].name}'.`,
            {
              code: 6051,
            },
          );
  ...
  let f = function (x, id, _ls, config = {}) {
    ...
    if (arr) {
      if (
        x.length > definition.params.length ||
        x.length < requiredLength
      ) {
        throw new ModelError(
          `Wrong number of parameters for ${fnName}.`,
          {
            code: 6052,
          },
        );
      ...
      if (!x.length && !definition.allowEmpty) {
        throw new ModelError(
          `At least one parameter required for ${name}().`,
          {
            code: 6053,
          },
        );
    ...
    for (let i = 0; i < x.length; i++) {
      ...
      function simplify(param) {
        ...
        if (
          !config.needPrimitive &&
          !config.allowOptionalPrimitive &&
          !config.needAgent &&
          !config.needPopulation
        ) {
          if (
            param instanceof SPrimitive &&
            !(param instanceof SPopulation)
          ) {
      ...
      if (
        config.noVector &&
        x[i] instanceof Vector
      ) {
        throw new ModelError(
          `${fnName} does not accept vectors for the parameter '${config.name}'.`,
          {
            code: 6055,
          },
        );
      ...
      if (
        ...
        throw new ModelError(
          `${fnName} does not accepted non-named vectors for the parameter '${config.name}'.`,
          {
            code: 6056,
          },
        );
      ...
      if (config.needVector) {
        ...
        if (!(x[i] instanceof Vector)) {
          throw new ModelError(
            `${fnName} requires a vector for the parameter '${config.name}'.`,
            {
              code: 6057,
            },
          );
      ...
      if (config.needNum) {
        ...
        if (!(x[i] instanceof Material)) {
          throw new ModelError(
            `${fnName} requires a number for the parameter '${config.name}'.`,
            {
              code: 6058,
            },
          );
      ...
      if (
        config.needPrimitive &&
        !(x[i] instanceof SPrimitive)
      ) {
        throw new ModelError(
          `${fnName} requires a primitive for the parameter '${config.name}'.`,
          {
            code: 6059,
          },
        );
      ...
      if (
        ...
        throw new ModelError(
          `${fnName} does not accept boolean values for the parameter '${config.name}'.`,
          {
            code: 6060,
          },
        );
      ...
      if (config.needString) {
        if (
          !(
            typeof x[i] === "string" ||
            x[i] instanceof String
          )
        ) {
          throw new ModelError(
            `${fnName} requires a string for the parameter '${config.name}'.`,
            {
              code: 6061,
            },
          );
      ...
      if (
        ...
        throw new ModelError(
          `${fnName} does not accept string values for the parameter '${config.name}'.`,
          {
            code: 6062,
          },
        );
    ...
    if (arr && definition.recurse) {
      ...
      if (
        definition.params[0] &&
        definition.params[0].leafNeedNum
      ) {
        if (!(q instanceof Vector)) {
          if (!(q instanceof Material)) {
            throw new ModelError(
              `${fnName} requires a number for the parameter '${definition.params[0].name}'.`,
              {
                code: 6058,
              },
            );
          ...
          if (v instanceof Vector && v.namesLC) {
            if (!base) {
              ...
              if (!base.keysMatch(v.namesLC)) {
                throw new ModelError(
                  `Vector keys do not match between parameters '${definition.params[baseI].name}' and '${definition.params[vectorized[i]].name}' in ${fnName}.`,
                  {
                    code: 6064,
                  },
                );
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          if (z instanceof Vector) {
            if (!base.keysMatch(z.namesLC)) {
              throw new ModelError(
                `Vector keys do not match between parameter '${definition.params[baseI].name}' and calculation result.`,
                {
                  code: 6065,
                },
              );
...
function objectizeFunction(fn) {
  let f = function (x, fingerprint, lastSelf) {
    if (!lastSelf) {
      throw new ModelError(
        "Object function not used on object",
        {
          code: 6066,
        },
      );
...
function factorial(x) {
  if (Math.round(x) !== x) {
    throw new ModelError(
      "The factorial() function only accepts integers.",
      {
        code: 6067,
      },
    );
    ...
    throw new ModelError(
      "The factorial() function is only defined for integers 0 or larger.",
      {
        code: 6068,
      },
    );
...
export function testArgumentsSize(
  x,
  name,
  min,
  max,
  ...
  if (x.length < min || x.length > max) {
    throw new ModelError(
      `Wrong number of parameters for ${name}().`,
      {
        code: 6069,
      },
    );
